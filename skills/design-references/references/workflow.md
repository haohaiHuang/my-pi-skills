...
8. **候选验证（pi 平台硬步骤：对选中的 2-3 个候选逐一验证，双引擎退化链——先 `hallmark_study_fetch <url>` 快验（秒级零依赖）；需要精确 token 直引 / 站点 JS 重或 SPA / 快验失败时，升级 `dembrandt <url> --design-md --save-output`（真浏览器渲染，产精确计算值 + google-labs 规范 DESIGN.md，落 `output/<domain>/`）；其他平台：WebFetch 或人工核对）**：拿真实字体/色值/间距/结构信号。验证成功（任一引擎拿到精确值）→ 该候选可"直引"（萃取具体数值进约束，dembrandt 验证的可直接回填参考台账）；验证失败/抓不到 → 标注"未验证"，只能"属性级借用"（如"暗色仪表盘式"），禁止把未验证候选当直引参考。验证结果一并展示给用户。
