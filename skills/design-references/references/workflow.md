...
3. **搜索失败时走决策树**（见下方「搜索失败决策框架」），每次落空按规则决定下一步，不碰运气
4. 记录每个候选的 token 草稿 + 理由（供环节 2 直接消费）
5. 主参考无匹配 → 次参考（Beautiful UI / Aceternity / 21st.dev）→ 兜底（minimal / Uiverse）
6. 候选给 2-3 个，禁多禁少；附一句取舍理由
