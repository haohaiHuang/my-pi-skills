...
4. 分支 A：refero 搜同品类真实产品 → 拿完整 DESIGN.md（仅在用户参考库无可匹配候选时）；分支 B：Zine 路由表定位风格族 → 读族详情
5. **搜索失败时走决策树**（见下方「搜索失败决策框架」）
6. 每个候选记录 token 草稿 + 理由（供环节 2 直接消费）
7. 候选给 2-3 个，禁多禁少；**附一句取舍理由 + 与已有资产的匹配度分析**
8. **展示给用户选（强制）**：用户选定方向后，才能进环节 2。禁止自行拍板。开放指令（用户未指定风格）时此步为硬门槛，候选必须来自用户参考库优先。
