...
3. 按分支问询缺口（见 SKILL.md 问询协议），已推断的跳过
4. 输出 Brief：格式 / 渠道 / 受众 / 目标 / 硬约束
5. **形态核对**：确认产物形态（Mac 应用 → 模拟窗口；网页 → 页面；海报 → 画布）——形态错误是最低级却最常见的错误
...
2. **候选必须下钻完整 DESIGN.md**：抓色板全表（每色标注 hex/角色）→ 字体规则 → 圆角/阴影规则 → Do/Don't 全清单。**禁止只看 northStar 一句话摘要就动手**（northStar 只用于筛选，不用于设计）
