...
**同一张图追问细节**（多轮会话，避免视觉模型每次"第一印象"）：把上一轮的回答作为 context 传入

```bash
vision-cli /图.png "概括布局"                      # 第一轮
vision-cli /图.png "左侧菜单有什么？" --context "上一轮的问答内容"   # 追问
```

**提取清单类内容**（结构化 JSON 输出）：

```bash
vision-cli /图.png "提取所有按钮文字" --format json
# → {"summary":"...","items":[{"text":"..."}]}
```
