...
| **4 校验** | 四段全跑：①机器层 `design_audit`（含动效 EM-* 子集 + kill-ai-slop 转译 KS-* 子集）②品牌层 Kami 三查 ③视觉层 hallmark 58 gates（视觉类按 slop-test.md 自查）+ 动效视觉自查（频率档/目的命名/慢放检查）④UX 层 design-qa-checklist。任一不达标 → 回环节 2 改约束，不打补丁 | design_audit / Kami 三查 / slop-test.md / design-qa-checklist | 达标/回炉 |
...
- **后置**（环节 4）：slop-test 58 gates 验收 + kill-ai-slop 转译子集（KS-*）—— 机器部分走 `design_audit`，视觉部分模型自查
