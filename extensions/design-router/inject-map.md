...
| **1 调研** | **硬步骤①：先调用 `design_research <branch> <query>`** 取候选池（本地台账 → refero 探测 → web 搜索，带证据来源）。**硬步骤②：对最终选中的 2-3 个候选，逐一调用 `hallmark_study_fetch <url>` 验证**（拿真实字体/色值/结构信号）；验证不了的候选标注"未验证"（证据等级降级为属性级借用，禁止当直引参考）。然后每个候选标注形态×气质×行业×防重四维 + 理由，**展示给用户选（强制）**；用户 go ahead 才静默。**禁止仅凭模型内建知识直接选风格** | 真实产品参考（用户参考库/refero/web 搜索）+ hallmark_study_fetch 验证 + Hallmark 形态库（21 宏结构）+ 气质库（4 genre/21 theme） | 风格候选（带证据） |
